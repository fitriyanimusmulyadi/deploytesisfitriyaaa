...
def predict(img):
    ...
    X = np.concatenate([hog_feat, mob_feat]).reshape(1, -1)

    X = scaler.transform(X)
    X = pca.transform(X)

    preds = np.array([m.predict(X[:, idx]) for m, idx in models])
...
st.write("Upload MRI image untuk deteksi tumor otak")
...
if uploaded_file is not None:
    ...
    st.image(img, channels="BGR", caption="Input MRI")
